...
// AdminSkills returns a handler for CRUD /admin/skills (GET list, POST create, PUT update, DELETE).
func AdminSkills(db *database.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPost:
			...
			if req.CategoryID <= 0 || req.Name == "" || req.Level == "" {
				RespondError(w, http.StatusBadRequest, "category_id, name and level required")
				return
			}
		...
		case http.MethodPut:
			...
			if req.ID <= 0 || req.CategoryID <= 0 || req.Name == "" || req.Level == "" {
				RespondError(w, http.StatusBadRequest, "id, category_id, name and level required")
				return
			}
